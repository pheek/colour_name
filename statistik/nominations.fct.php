 ...
 * Dies sind natürlich viel weniger als die Nominationen,
 * denn viele Namen werden mehrfach vergeben.
 *
 * @global Database $TPL_DB
 ...
 * Genau genommen, wird das Quadrat des Pythagoras (Raumdiensionale)
 * im Quader genommen; dies ist äquivalent zur Wurzel, wenn nur die 
 * Wurzel bei gar keinem Abstand gezogen wird.
 ...
 */
	function getNearestColours($r, $g, $b, $lang="") {
		...
		$sel = 'SELECT * FROM (SELECT `farbnamen`.`Name`, count(`farbnamen`.`Name`) as cnt,'.
		       ...
		       'WHERE `farbnamen`.`ID` = `nomination`.`F_farbnamen` '.
		       'AND `nomination`.`F_rgb` = `rgb`.`ID` AND '.
